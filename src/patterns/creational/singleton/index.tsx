...
function Session({ label, isConnected, setIsConnected }: SessionProps) {
  ...
  return (
    <div className="bg-zinc-800 border border-zinc-700 p-6 rounded-xl">
      <h2 className="text-emerald-400 text-xl mb-4 border-b border-zinc-700 pb-4">{label}</h2>
      <div className="flex flex-col gap-3 mb-6">
        <button className="bg-emerald-600 text-white px-4 py-2 rounded-lg hover:bg-emerald-500 cursor-pointer" onClick={handleConnect}>
          Conectar
        </button>
        <select className="bg-zinc-900 border border-zinc-600 text-zinc-100 px-4 py-2 rounded-lg w-full" value={selectedQuery} onChange={e => setSelectedQuery(e.target.value)}>
          {QUERIES.map(q => (
            <option key={q}>{q}</option>
          ))}
        </select>
        <button className="bg-transparent border border-zinc-500 text-zinc-300 hover:border-emerald-500 hover:text-emerald-400 px-4 py-2 rounded-lg cursor-pointer" onClick={handleQuery}>
          Executar Query
        </button>
      </div>
      <div className="flex items-center gap-2 bg-zinc-900 rounded-lg px-4 py-2 mb-6">
        ...
        <span className="text-zinc-400 text-sm">{isConnected ? 'Conectado' : 'Desconectado'}</span>
      </div>
      <ul className="bg-zinc-950 max-h-40 overflow-y-auto p-4 rounded-lg border border-zinc-700 font-mono text-sm text-zinc-400 ">
        {logs.map((log, index) => (
